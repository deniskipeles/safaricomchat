<template>
  <div>
    <div :style="styles.wrapper">
      <div style="margin: auto">
        <p :style="styles.title">Machakos University Social Page</p>
        <!-- <p :style="styles.subtitle">Login with one of our sample users</p> -->
        <div :style="styles.userWrapper">
          <div
            :key="i"
            :style="styles.userSelector"
            @click="login($event, name)"
            v-for="({ name, image }, i) in users"
          >
            <div :style="styles.avatarWrapper">
              <comet-chat-avatar :image="image" />
            </div>

            <p style="margin: auto">
              <a href="javascript:;">{{ name }}</a>
            </p>
          </div>
        </div>

        <p :style="styles.loginText">Login with Username</p>

        <input
          type="text"
          v-model="uid"
          :style="styles.input"
          placeholder="Enter your Username or email here"
          @keyup.enter="login($event, uid)"
        />
        <input
          type="password"
          v-model="password"
          :style="styles.input"
          placeholder="Enter your password"
        />

        <div :style="styles.loginButton" @click="login($event, uid)">Login</div>
        <hr/>
        <p style="margin: auto">
          <a href="/register">Register</a>
        </p>
      </div>

    </div>
    <loader v-if="showloader" />
  </div>
</template>
<script>
import { CometChat } from "@cometchat-pro/chat";
import { CometChatAvatar } from "../../cometchat-pro-vue-ui-kit/CometChatWorkspace/src";
import PocketBase from 'pocketbase';
import { COMETCHAT_CONSTANTS } from "../../CONSTS";
import Loader from "../Loader/Loader";

import * as style from "./style";

export default {
  name: "AppHome",
  components: {
    Loader,
    CometChatAvatar,
  },
  data() {
    return {
      users: [],
      // [
      //   {
      //     name: "superhero1",
      //     image:
      //       "https://data-us.cometchat.io/assets/images/avatars/ironman.png",
      //   },
      //   {
      //     name: "superhero2",
      //     image:
      //       "https://data-us.cometchat.io/assets/images/avatars/captainamerica.png",
      //   },
      //   {
      //     name: "superhero3",
      //     image:
      //       "https://data-us.cometchat.io/assets/images/avatars/spiderman.png",
      //   },
      //   {
      //     name: "superhero4",
      //     image:
      //       "https://data-us.cometchat.io/assets/images/avatars/wolverine.png",
      //   },
      //   {
      //     name: "superhero5",
      //     image:
      //       "https://data-us.cometchat.io/assets/images/avatars/cyclops.png",
      //   },
      // ],
      showloader: false,
      uid: null,
      password:null
    };
  },
  computed: {
    styles() {
      return {
        wrapper: style.wrapperStyle(),
        title: style.titleStyle(),
        subtitle: style.subtitleStyle(),
        userWrapper: style.userWrapperStyle(),
        userSelector: style.userSelectorStyle(),
        avatarWrapper: style.avatarWrapperStyle(),
        loginText: style.loginTextStyle(),
        input: style.inputStyle(),
        loginButton: style.loginButtonStyle(),
      };
    },
  },
  methods: {
    async login(event, uid) {
      if (event) {
        event.preventDefault();
      }

      if (uid) {
        this.uid = uid;
      } else {
        alert("Please enter valid UID");
        return;
      }

      const client = new PocketBase('https://bnet.fly.dev');


      // fetch a paginated records list
      const filter = `username = "${this.uid}" && password = "${this.password}" || email = "${this.uid}" && password = "${this.password}"`
      const resultList = await client.records.getList('auth_users', 1, 2, {
          filter,
      });

      if (resultList.items.length > 0) {
        const username = resultList.items[0].username
        CometChat.login(username, COMETCHAT_CONSTANTS.AUTH_KEY)
          .then(() => {
            location.href = "/chats";
          })
          .catch((error) => {
            console.log("Error in [login]", error);
            alert((error || {}).message || "Error in login");
          })
          .finally(() => {
            this.showloader = false;
          });
        
          this.showloader = false;
        } else {
          this.showloader = false;
          
        }

    },
  },
  created() {
    this.showloader = true;
    CometChat.getLoggedinUser().then(
      (user) => {
        if (user) {
          location.href = "/chats";
          this.uid = user.getUid();
        } else {
          this.showloader = false;
        }
      },
      (error) => {
        console.log("Error in [getLoggedinUser]", error);
        this.showloader = false;
      }
    );
  },
};
</script>